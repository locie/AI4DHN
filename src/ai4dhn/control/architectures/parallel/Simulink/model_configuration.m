function cs = model_configuration()
% MATLAB function for configuration set generated on 07-Jul-2026 12:57:16
...
cs.set_param('StopTime', '150000');   % Stop time
...
cs.set_param('FixedStep', '60');   % Fixed-step size (fundamental sample time)
...
cs.set_param('OptimizationPriority', 'Speed');   % Priority
...
cs.set_param('ArtificialAlgebraicLoopMsg', 'warning');   % Minimize artificial algebraic loop occurrences
...
cs.set_param('CustomCodeGlobalsAsFunctionIO', 'off');   % Automatically infer global variables as function interfaces
...
cs.set_param('HalideCodeGeneration', 'off');   % Generate Halide code
...
cs.set_param('CodeProfilingInstrumentation', 'coarse');   % Measure function execution times
...
cs.set_param('LargeConstantGeneration', 'KeepInSourceFiles');   % Large constant generation in MATLAB functions
...
cs.set_param('ParamNamingRule', 'UpperCase');   % Parameter naming
...
cs.set_param('BlockCommentType', 'BlockSIDComment');   % Trace to model using
...
cs.set_param('LogToMDFFile', 'off');   % Log signals to MDF file
...
cs.set_param('RateTransitionBlockCode', 'Function');   % Rate Transition block code
...
cs.set_param('SuppressUnreachableDefaultCases', 'on');   % Suppress generation of default cases for switch statements if unreachable
...
try
  ...
  cs.set_param('EditingMode', 'Full');   % Editing Mode
  cs.set_param('ExplicitSolverDiagnosticOptions', 'warning');   % Explicit solver used in model containing Physical Networks blocks
  cs.set_param('GlobalZcOffDiagnosticOptions', 'warning');   % Zero-crossing control is globally disabled in Simulink
  cs.set_param('SimscapeNormalizeSystem', 'on');   % Normalize using nominal values
  cs.set_param('SimscapeNominalValues', '[{"value":"1","unit":"A"},{"value":"1","unit":"bar"},{"value":"1","unit":"cm^2"},{"value":"1","unit":"cm^3/s"},{"value":"1","unit":"kJ/kg"},{"value":"1","unit":"kW"},{"value":"1","unit":"l"},{"value":"1","unit":"N"},{"value":"1","unit":"N*m"},{"value":"1","unit":"V"}]');   % Specify nominal values...
  cs.set_param('SimscapeLogType', 'none');   % Log simulation data
  cs.set_param('SimscapeUseOperatingPoints', 'off');   % Enable operating point initialization
  cs.set_param('SimscapeCompileComponentReuse', 'off');   % Reuse components during compilation
  cs.set_param('SimscapeCompileArtifactsDiskCache', 'off');   % Store cached compilation artifacts on disk
  cs.set_param('SimscapeMultithreadedCompilation', 'on');   % Enable multithreaded compilation
  cs.set_param('SimMechanicsInvalidVisualProperty', 'warning');   % Invalid visual properties
  cs.set_param('SimMechanicsCrossSectionNullEdge', 'warning');   % Repeated vertices in a cross-section
  cs.set_param('SimMechanicsUnconnectedFramePorts', 'warning');   % Unconnected frame port
  cs.set_param('SimMechanicsUnconnectedGeometryPorts', 'warning');   % Unconnected geometry port
  cs.set_param('SimMechanicsRedundantBlock', 'warning');   % Redundant block
  cs.set_param('SimMechanicsConflictingReferenceFrames', 'warning');   % Conflicting reference frames
  cs.set_param('SimMechanicsRigidlyBoundBlock', 'error');   % Rigidly constrained block
  cs.set_param('SimMechanicsUnsatisfiedHighPriorityTargets', 'warning');   % Unsatisfied high priority state targets
  cs.set_param('SimMechanicsJointTargetOverSpecification', 'error');   % Overspecified targets in kinematic loops
  cs.set_param('SimMechanicsOpenEditorOnUpdate', 'on');   % Open Mechanics Explorer on model update or simulation
catch ME
  warning('Simulink:ConfigSet:AttachComponentError', '%s', ME.message);
end